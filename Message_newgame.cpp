...
/*
 * 处理11000号报文
 * 新建游戏:11000+长度+TOKEN+队伍编号
 */
MsgNewGame::MsgNewGame(const int &fd, \
                   const int &cmd, \
                   char *msg, \
                   const int &msg_size):Message(fd, cmd, msg, msg_size)
{}

int MsgNewGame::doRequest()
{
    int ret;
    int flag = CMD_SIZE;
    ...
    int token_size = 0;
    {
        ...
        token_size = *( (int *)data );
        if( m_msgSize != CMD_SIZE + INT_SIZE + token_size + INT_SIZE + TIMESTAMP_SIZE + CK_SIZE )
        {
    ...
    string url_resp = "";
    {
        ...
        string querystr = "";
        querystr = querystr + "tk=" + token + "&teamno=" + N2S(teamno);
