...
string cwsURL(const string &url, const string &querystr, const string &cwsKey)
{
    ...
    // 生成ck
    string tmpstr = querystr + "&ts=" + N2S( timestamp ) + cwsKey;
    string myCK = "";
    iMD5( tmpstr.c_str(), tmpstr.size(), myCK );
    myCK = myCK.substr(16, 16);

    // 加上ck，生成最终的url
    return url + "?" + querystr + "&ts=" + N2S( timestamp ) + "&ck=" + myCK;
...
int iMD5( const char* key, const int &len, string &sMD5 )
{
    ...
    //len = strlen( key );
    //MB_LOGGER_DEBUG("[MD5] message length %d", len);
    MD5_Init( &context );
    MD5_Update( &context, (unsigned char *)key, len );
    MD5_Final( &context, buff );
    
    for(j=0;j<16;j++)
    {
        //printf("%x",(buff[j] & 0xF0)>>4);
        sprintf(output, "%x", (buff[j] & 0xF0)>>4);
        output[1] = '\0';
        sMD5 += output;
    
        //printf("%x",buff[j] & 0x0F);
        sprintf(output, "%x", buff[j] & 0x0F);
        output[1] = '\0';
        sMD5 += output;
    }
    //MB_LOGGER_DEBUG("[MD5] %s", sMD5.c_str());
...
string param2string(const map<string, string> &req, const map<string, string> &resp)
{
    ...
    msg << " req{" << pack2string(req) << " } "
        << " resp{" << pack2string(resp) << " }";
...
{
    ostringstream msg;
    vector< map< string, string > >::const_iterator list_it = list.begin();
    for (; list_it != list.end(); ++list_it)
    {
        map<string, string>::const_iterator it = list_it->begin();
...
int TimeStst::Diff(struct timeval &diff)
{
    if(m_begin.tv_sec > m_end.tv_sec)
    {
        return -1;
    }

    if( (m_begin.tv_sec == m_end.tv_sec) && (m_begin.tv_usec > m_end.tv_usec) )
    {
        return -1;
    }

    diff.tv_sec   = m_end.tv_sec - m_begin.tv_sec;
    diff.tv_usec  = m_end.tv_usec - m_begin.tv_usec;
